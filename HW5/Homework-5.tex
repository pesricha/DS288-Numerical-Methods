...
\begin{tabular}{ccccccc}
\hline
\textbf{Integral} & \( R_{n,n} \) & \( n \) & \textbf{True Value} & \( \epsilon_{n,n} \) & \#Function Evaluations \\
\hline
...
\begin{tabular}{ccccccc}
\hline
\textbf{Integral} & \( n \) & \( R_{n,0} \) & \textbf{True Value} & \( \epsilon_{n,0} \) & \( h \) \\
\hline
...
\begin{minipage}{0.48\textwidth}
    It is also noticed that \( \int_0^1 x^{1/3} \, dx \) converged slower than \( \int_0^1 x^2 e^{-x} \, dx \) even though the former has a simpler form. This is because Rhomberg Integration is based on trapezoidal rule, which integrates the function by linearly interpolating points $(x_{i}, f_{i})$ and $(x_{i+1}, f_{i+1})$. Hence it is very good at approximating the integral when the function in which the change in derivative is less in the integrating region.\\ \\ 
    As evident from the figure, the rate of change of derivative in $x^{1/3}$ is very high for the first half and hence it takes more iterations to converge.
\end{minipage}%
\hfill % Space between the minipages
\begin{minipage}{0.46\textwidth}
    % Right minipage with an image
    \centering
    \includegraphics[width=\textwidth]{example-image} % Replace with actual image file
