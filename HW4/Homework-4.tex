...
\end{tcolorbox}
\begin{tcolorbox}
    The values obtained for both ($\frac{d\phi}{d\theta})_{Forward}$ and ($\frac{d\phi}{d\theta})_{Centered}$ almost overlap with each other. \textbf{We expect ($\frac{d\phi}{d\theta})_{Centered}$ to be more accurate as the error term for center difference is $\frac{h^2}{6} f^{(3)}(x)$ and that of forward difference is $\frac{h}{2} f''(x)$}.
    \\
     
\end{tcolorbox} 

\begin{tcolorbox}
If we assume the centre value to be true, and plot $\Delta = $$|(d\phi/d\theta)_{C}$ - $(d\phi/d\theta)_{FW}|$, \textbf{we find that the minimas of $\Delta$ coincide with points where derivative of $\frac{d\phi}{d\theta}$ zero.} This checks out with the fact as as the \textbf{error term has a factor of second derivative => $f''(x) = 0$. and at those points both center and forward have the same error term.}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{HW4/imagesmall/compare center-fwd phi.png}
...
\begin{center}
    ...
    Then we can solve for $\beta$ using the same method that we used for Q1.
\end{center}
...
\begin{tcolorbox}
    From the values in table 1, it's evident that at $\theta = 100^o$, Angular Velocity is close to 10 rad/sec and Angular Acceleration is close to -25 rad/sec$^2$.
    \begin{figure}[H]
        \centering
