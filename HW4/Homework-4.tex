...
\begin{tcolorbox}
If we assume the centre value to be true, and plot $\Delta = $$|(d\phi/d\theta)_{C}$ - $(d\phi/d\theta)_{FW}|$, \textbf{we find that the minimas of $\Delta$ coincide with points where derivative of $\frac{d\phi}{d\theta}$ is close to zero.} This checks out with the fact as as the \textbf{error term has a factor of second derivative and at those points they should have the same error behaviour.}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{HW4/imagesmall/centered-fwd.png}
